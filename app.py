...
comfy_image = (
    ...
    .run_commands(
        # Clone ComfyUI
        f"cd {REMOTE_BASE_PATH} && git clone https://github.com/comfyanonymous/ComfyUI.git",
        ...
        f"cd {COMFYUI_PATH}/custom_nodes/ComfyUI-WanVideoWrapper && pip install --no-cache-dir -r requirements.txt || true",
        ...
        f"cd {COMFYUI_PATH}/custom_nodes/ComfyUI-VideoHelperSuite && pip install --no-cache-dir -r requirements.txt || true",
        # Install ControlNet Aux
        f"cd {COMFYUI_PATH}/custom_nodes && git clone https://github.com/Fannovel16/comfyui_controlnet_aux.git",
        f"cd {COMFYUI_PATH}/custom_nodes/comfyui_controlnet_aux && pip install --no-cache-dir -r requirements.txt || true",
    )
    .pip_install(
        ...
        "torch>=2.0.0",
        "torchvision>=0.15.0",
        "huggingface-hub>=0.19.0",  # untuk HF auth
    )
)
