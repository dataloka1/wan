...
class ComfyUI:
    ...
    def generate_i2v(
        ...
        if use_fast_mode and high_lora:
            workflow["101"] = {"class_type": "LoraLoaderModelOnly", "inputs": {"lora_name": high_lora, "strength_model": 1.0, "model": ["37", 0]}}
            workflow["102"] = {"class_type": "LoraLoaderModelOnly", "inputs": {"lora_name": low_lora, "strength_model": 1.0, "model": ["56", 0]}}
        ...
        workflow.update({
            "54": {"class_type": "ModelSamplingSD3", "inputs": {"shift": shift_high, "model": high_model_source}},
            "55": {"class_type": "ModelSamplingSD3", "inputs": {"shift": shift_low, "model": low_model_source}},
            ...
            "61": {"class_type": "SaveVideo", "inputs": {"filename_prefix": "i2v", "format": "video/h264-mp4", "video": ["109", 0]}}
        })
        prompt_id = self._queue_prompt(client_id, workflow)
        try:
            video_bytes = self._get_video_from_websocket(prompt_id, client_id)
            ...
            print(f"[FALLBACK] WebSocket retrieval failed: {e}")
            print("[FALLBACK] Attempting filesystem retrieval...")
            video_bytes = self._get_video_from_filesystem(prompt_id, "i2v")
        return base64.b64encode(video_bytes).decode('utf-8')
    ...
    def generate_animate(
        ...
        workflow.update({
            ...
            "19": {"class_type": "SaveVideo", "inputs": {"filename_prefix": "animate", "format": "video/h264-mp4", "video": ["15", 0]}}
        })
        prompt_id = self._queue_prompt(client_id, workflow)
        try:
            video_bytes = self._get_video_from_websocket(prompt_id, client_id)
            ...
            print(f"[FALLBACK] WebSocket retrieval failed: {e}")
            print("[FALLBACK] Attempting filesystem retrieval...")
            video_bytes = self._get_video_from_filesystem(prompt_id, "animate")
        return base64.b64encode(video_bytes).decode('utf-8')
    ...
    def apply_camera_lora(
        ...
        workflow = {
            ...
            "8": {"class_type": "VAEDecode", "inputs": {"samples": ["58", 0], "vae": ["39", 0]}},
            "109": {"class_type": "CreateVideo", "inputs": {"fps": 16, "images": ["8", 0]}},
            "61": {"class_type": "SaveVideo", "inputs": {"filename_prefix": f"camera_{camera_motion}", "format": "video/h264-mp4", "video": ["109", 0]}}
        }
        prompt_id = self._queue_prompt(client_id, workflow)
        try:
            video_bytes = self._get_video_from_websocket(prompt_id, client_id)
            ...
            print(f"[FALLBACK] WebSocket retrieval failed: {e}")
            print("[FALLBACK] Attempting filesystem retrieval...")
            video_bytes = self._get_video_from_filesystem(prompt_id, f"camera_{camera_motion}")
        return base64.b64encode(video_bytes).decode('utf-8')
