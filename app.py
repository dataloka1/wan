...
class ComfyUI:
    ...
    def _get_video_from_websocket(self, prompt_id: str, client_id: str) -> bytes:
        ...
        while time.time() - poll_start_time < POLL_TIMEOUT:
            ...
            time.sleep(1)
        raise ValueError("No video output found after polling.")
    ...
    def generate_t2v(
        ...
        workflow.update({
            ...
            "80": {"class_type": "SaveVideo", "inputs": {"filename_prefix": "video/t2v", "format": "auto", "codec": "auto", "video": ["88", 0]}}
        })
    ...
    def generate_i2v(
        ...
        workflow.update({
            ...
            "61": {"class_type": "SaveVideo", "inputs": {"filename_prefix": "video/i2v", "format": "auto", "codec": "auto", "video": ["109", 0]}}
        })
        prompt_id = self._queue_prompt(client_id, workflow)
        video_bytes = self._get_video_from_websocket(prompt_id, client_id)
        return base64.b64encode(video_bytes).decode('utf-8')
