...
def setup_comfyui():
    ...
    if stats["failed"] > 0:
        print(f"WARNING: {stats['failed']} models failed to download.")
...
class QueueManager:
    ...
    def _ensure_connection(self):
        ...
        try:
            ...
            print(f"[REDIS] Connection lost, reconnecting: {e}")
            try:
                self.redis = get_redis_client()
            except Exception as e2:
                print(f"[REDIS] Reconnection failed: {e2}")
    ...
    def enqueue_task(self, task_data: Dict) -> str:
        ...
        try:
            ...
            print(f"[QUEUE] Failed to enqueue task: {e}")
            raise
    ...
    def dequeue_task(self) -> Optional[Dict]:
        ...
        self._ensure_connection()
        try:
            result = self.redis.brpop(REDIS_QUEUE_KEY, timeout=1)
            if result:
                _, task_json = result
            ...
            print(f"[QUEUE] Error dequeuing: {e}")
        return None
    ...
    def update_task_status(self, task_id: str, status: GenerationStatus, **kwargs):
        ...
        try:
            ...
            print(f"[QUEUE] Failed to update status: {e}")
    ...
    def update_progress(self, task_id: str, progress_data: Dict):
        ...
        try:
            ...
            print(f"[QUEUE] Failed to update progress: {e}")
    ...
    def get_queue_status(self) -> Dict:
        ...
        try:
            ...
            print(f"[QUEUE] Failed to get status: {e}")
            return {
                'queue_length': 0,
                'active_tasks': 0,
                'max_concurrent': MAX_CONCURRENT_REQUESTS,
    ...
    def cancel_task(self, task_id: str) -> bool:
        ...
        try:
            ...
            print(f"[QUEUE] Failed to cancel task: {e}")
...
class ComfyUI:
    @enter()
    def startup(self):
        """Initialize container and start ComfyUI server"""
    ...
    def _signal_handler(self, signum, frame):
        ...
        print(f"\n[SHUTDOWN] Container {self.container_id} received signal {signum}")
        self._cleanup()
        sys.exit(0)
    ...
    def _send_webhook_with_retry(self, webhook_url: str, task_id: str, status: str, 
        ...
        for attempt in range(max_retries):
            try:
                print(f"[WEBHOOK] Attempt {attempt + 1}/{max_retries}: Sending {status} for {task_id}")
                ...
                print(f"[WEBHOOK] ✓ Successfully sent {status} for {task_id}")
                return True
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"[WEBHOOK] ✗ All attempts failed for {task_id}")
                    return False
        ...
        for attempt in range(DEFAULT_RETRY_COUNT):
            try:
                ...
                if attempt < DEFAULT_RETRY_COUNT - 1:
                    ...
                    raise RuntimeError(f"Failed to get history: {str(e)}")

    def _get_file(self, filename: str, subfolder: str, folder_type: str) -> bytes:
        """Get file from ComfyUI"""
        ...
        for attempt in range(DEFAULT_RETRY_COUNT):
            try:
                ...
                if attempt < DEFAULT_RETRY_COUNT - 1:
                    ...
                    raise RuntimeError(f"Failed to get file: {str(e)}")

    def _get_video_from_websocket(self, task: GenerationTask, total_steps: int = 30) -> bytes:
        """Get video from ComfyUI via WebSocket"""
        ...
        try:
            ...
            if not generation_done:
                if time.time() - start_time >= dynamic_timeout:
                    ...
                    self._update_progress(task.task_id, GenerationStatus.FAILED, 0.0, "Generation failed")
            ...
            while time.time() - poll_start_time < poll_timeout:
                ...
                if consecutive_empty_results > 3:
                    ...
                    poll_interval = max(poll_interval * 0.9, DEFAULT_POLL_INTERVAL)
                
                print(f"[POLL] Attempt {poll_count}, elapsed: {elapsed:.1f}s")
                ...
                try:
                    ...
                    print(f"[POLL] Status: {status_str}")
                    
                    if status_str == 'error':
                        error_info = status.get('messages', [])
                        self._update_progress(task.task_id, GenerationStatus.FAILED, 0.0, "Generation failed", str(error_info))
                    ...
                    for node_id, node_output in outputs.items():
                        for media_type in ['videos', 'gifs', 'images']:
                            if media_type in node_output and node_output[media_type]:
                                ...
                                print(f"[POLL] Found {len(items)} {media_type}")
                                ...
                                for item in items:
                                    ...
                                    print(f"[OUTPUT] Downloading: {filename}")
                                    
                                    try:
                                        data = self._get_file(filename, subfolder, file_type)
                                        if len(data) > 1024:
                                            print(f"[OUTPUT] Successfully got {len(data)/1024/1024:.2f} MB")
                                            self._update_progress(task.task_id, GenerationStatus.COMPLETED, 100.0, "Video ready")
                                            return data
                                    except Exception as e:
            ...
            self._update_progress(task.task_id, GenerationStatus.FAILED, 0.0, f"Error: {str(e)}")
            raise
    ...
    def _create_t2v_workflow(self, prompt: str, negative_prompt: str, width: int, height: int, 
        ...
        workflow = {
            ...
            "2": {"inputs": {"text": negative_prompt, "clip": ["4", 1]}, "class_type": "CLIPTextEncode"},
            "3": {"inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler", "scheduler": "normal", "denoise": 1.0, "model": ["5", 0], "positive": ["1", 0], "negative": ["2", 0], "latent_image": ["6", 0]}, "class_type": "KSampler"},
            "4": {"inputs": {"text_encoder": ["7", 0], "vae": ["8", 0]}, "class_type": "WanCLIP"},
            "5": {"inputs": {"model": high_noise_model, "low_noise_model": low_noise_model, "high_lora": high_lora, "low_lora": low_lora}, "class_type": "WanModelLoader"},
            "6": {"inputs": {"width": width, "height": height, "num_frames": num_frames, "batch_size": 1}, "class_type": "EmptyLatentVideo"},
            "7": {"inputs": {"text_encoder": "umt5_xxl_fp8_e4m3fn_scaled.safetensors"}, "class_type": "WanTextEncoderLoader"},
            "8": {"inputs": {"vae": "wan_2.1_vae.safetensors"}, "class_type": "WanVAELoader"},
            "9": {"inputs": {"samples": ["3", 0], "vae": ["8", 0]}, "class_type": "VAEDecode"},
            "10": {"inputs": {"filename_prefix": "wan_t2v", "images": ["9", 0]}, "class_type": "SaveAnimatedWEBP"}
        }
    ...
    def _create_i2v_workflow(self, image_path: str, prompt: str, negative_prompt: str, width: int, height: int,
        ...
        workflow = {
            ...
            "4": {"inputs": {"text_encoder": ["7", 0], "vae": ["8", 0]}, "class_type": "WanCLIP"},
            "5": {"inputs": {"model": high_noise_model, "low_noise_model": low_noise_model, "high_lora": high_lora, "low_lora": low_lora}, "class_type": "WanModelLoader"},
            "6": {"inputs": {"image": ["9", 0], "vae": ["8", 0]}, "class_type": "ImageToLatent"},
            "7": {"inputs": {"text_encoder": "umt5_xxl_fp8_e4m3fn_scaled.safetensors"}, "class_type": "WanTextEncoderLoader"},
            "8": {"inputs": {"vae": "wan_2.1_vae.safetensors"}, "class_type": "WanVAELoader"},
            "9": {"inputs": {"image": image_path}, "class_type": "LoadImage"},
            "10": {"inputs": {"samples": ["3", 0], "vae": ["8", 0]}, "class_type": "VAEDecode"},
            "11": {"inputs": {"filename_prefix": "wan_i2v", "images": ["10", 0]}, "class_type": "SaveAnimatedWEBP"}
        }
    ...
    def _create_animate_workflow(self, reference_image_path: str, video_path: str, prompt: str, negative_prompt: str,
        ...
        workflow = {
            ...
            "3": {"inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler", "scheduler": "normal", "denoise": 1.0, "model": ["5", 0], "positive": ["1", 0], "negative": ["2", 0], "latent_image": ["6", 0]}, "class_type": "KSampler"},
            "4": {"inputs": {"text_encoder": ["7", 0], "vae": ["8", 0]}, "class_type": "WanCLIP"},
            "5": {"inputs": {"model": "Wan2_2-Animate-14B_fp8_e4m3fn_scaled_KJ.safetensors", "low_noise_model": "", "high_lora": "", "low_lora": ""}, "class_type": "WanModelLoader"},
            "6": {"inputs": {"reference_image": ["9", 0], "video": ["10", 0], "vae": ["8", 0]}, "class_type": "VideoToLatent"},
            "7": {"inputs": {"text_encoder": "umt5_xxl_fp8_e4m3fn_scaled.safetensors"}, "class_type": "WanTextEncoderLoader"},
            "8": {"inputs": {"vae": "wan_2.1_vae.safetensors"}, "class_type": "WanVAELoader"},
            "9": {"inputs": {"image": reference_image_path}, "class_type": "LoadImage"},
            "10": {"inputs": {"video": video_path}, "class_type": "LoadVideo"},
            "11": {"inputs": {"samples": ["3", 0], "vae": ["8", 0]}, "class_type": "VAEDecode"},
            "12": {"inputs": {"filename_prefix": "wan_animate", "images": ["11", 0]}, "class_type": "SaveAnimatedWEBP"}
        }
    ...
    def _create_camera_lora_workflow(self, image_path: str, prompt: str, negative_prompt: str, camera_motion: str,
        ...
        workflow = {
            ...
            "3": {"inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler", "scheduler": "normal", "denoise": 1.0, "model": ["5", 0], "positive": ["1", 0], "negative": ["2", 0], "latent_image": ["6", 0]}, "class_type": "KSampler"},
            "4": {"inputs": {"text_encoder": ["7", 0], "vae": ["8", 0]}, "class_type": "WanCLIP"},
            "5": {"inputs": {"model": "wan2.2_i2v_high_noise_14B_fp8_scaled.safetensors", "low_noise_model": "wan2.2_i2v_low_noise_14B_fp8_scaled.safetensors", "high_lora": lora_filename, "low_lora": ""}, "class_type": "WanModelLoader"},
            "6": {"inputs": {"image": ["9", 0], "vae": ["8", 0]}, "class_type": "ImageToLatent"},
            "7": {"inputs": {"text_encoder": "umt5_xxl_fp8_e4m3fn_scaled.safetensors"}, "class_type": "WanTextEncoderLoader"},
            "8": {"inputs": {"vae": "wan_2.1_vae.safetensors"}, "class_type": "WanVAELoader"},
            "9": {"inputs": {"image": image_path}, "class_type": "LoadImage"},
            "10": {"inputs": {"samples": ["3", 0], "vae": ["8", 0]}, "class_type": "VAEDecode"},
            "11": {"inputs": {"filename_prefix": f"wan_camera_{camera_motion}", "images": ["10", 0]}, "class_type": "SaveAnimatedWEBP"}
        }
...
async def api_generate_t2v(request: T2VRequest):
    ...
    comfyui = ComfyUI()
    result = comfyui.generate_t2v.remote(request)
    return result
...
async def api_generate_i2v(request: I2VRequest):
    ...
    task_id = queue_manager.enqueue_task(request.dict())
    comfyui = ComfyUI()
    result = comfyui.generate_i2v.remote(request)
    return result
...
async def api_generate_animate(request: AnimateRequest):
    ...
    task_id = queue_manager.enqueue_task(request.dict())
    comfyui = ComfyUI()
    result = comfyui.generate_animate.remote(request)
    return result

@fastapi_app.post("/generate/camera_lora")
async def api_generate_camera_lora(request: CameraLoraRequest):
    """Generate video with camera motion LoRA"""
    task_id = queue_manager.enqueue_task(request.dict())
    comfyui = ComfyUI()
    result = comfyui.generate_camera_lora.remote(request)
    return result
