...
class ComfyUI:
    ...
    def _create_task(self, task_id: str, webhook_url: Optional[str] = None) -> GenerationTask:
    ...
    def generate_t2v(
        ...
        task = self._create_task(task_id, webhook_url)
...
# Create global ComfyUI instance
...
async def api_generate_t2v(request: T2VRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return {"status": "accepted", "message": "T2V generation started", "task_id": request.task_id}
    except Exception as e:
...
async def api_generate_i2v(request: I2VRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return {"status": "accepted", "message": "I2V generation started", "task_id": request.task_id}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def api_generate_animate(request: AnimateRequest, background_tasks: BackgroundTasks):
    ...
    try:
        # Run generation in background
        background_tasks.add_task(
            comfyui_instance.generate_animate,
        ...
        return {"status": "accepted", "message": "Animation generation started", "task_id": request.task_id}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@fastapi_app.post("/api/generate/camera-lora")
async def api_apply_camera_lora(request: CameraLoraRequest, background_tasks: BackgroundTasks):
    """API endpoint for camera motion LoRA"""
    try:
        # Run generation in background
        background_tasks.add_task(
            comfyui_instance.apply_camera_lora,
            image_base64=request.image_base64,
            prompt=request.prompt,
        ...
        return {"status": "accepted", "message": "Camera LoRA application started", "task_id": request.task_id}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def api_cancel_generation(request: CancelRequest):
    ...
    try:
        success = comfyui_instance.cancel_generation(request.task_id)
        if success:
...
async def api_get_task_status(task_id: str):
    ...
    try:
        status = comfyui_instance.get_task_status(task_id)
        if status:
            return status
        else:
...
async def health_check():
    ...
    try:
        ...
        return {"status": "unhealthy", "comfyui": "not_running"}
