...
class ComfyUI:
    ...
    def _queue_prompt(self, client_id: str, prompt_workflow: dict):
        ...
        print(f"[QUEUE] Mengirim workflow ke ComfyUI...")
        print(f"[QUEUE] Payload size: {len(payload_json)/1024:.2f} KB")
        ...
        try:
            ...
            print(f"[QUEUE] [ERROR] HTTP {e.code}: {error_body}")
    ...
    def _get_video_from_websocket(self, prompt_id: str, client_id: str):
        ...
        if prompt_id not in history:
            raise ValueError(f"History untuk prompt_id {prompt_id} tidak ditemukan")
            ...
            for video_type in ['videos', 'gifs']:
                if video_type in node_output:
                    for item in node_output[video_type]:
                        ...
                        video_bytes = self._get_file(item['filename'], item['subfolder'], item['type'])
                        return video_bytes
    ...
    def generate_video(
        ...
        # Base workflow yang DISEDERHANAKAN tanpa WanTextToVideo custom node
        # Kita gunakan node standar ComfyUI yang pasti ada
        workflow = {
            "4": {
        ...
        if loras:
            ...
            workflow["3"]["inputs"]["model"] = [final_model_id, 0]
            workflow["6"]["inputs"]["clip"] = [final_clip_id, 1]
            workflow["7"]["inputs"]["clip"] = [final_clip_id, 1]
        ...
        try:
            ...
            print(f"[WORKFLOW] Prompt queued dengan ID: {prompt_id}")
            
            # Ambil video dari websocket
            video_bytes = self._get_video_from_websocket(prompt_id, client_id)
            
            # Convert ke base64
            video_base64 = base64.b64encode(video_bytes).decode('utf-8')
            print(f"[WORKFLOW] [OK] Video berhasil di-generate ({len(video_base64)/1024:.2f} KB base64)")
            
            return video_base64
            
        except Exception as e:
            error_msg = f"Error dalam workflow generation: {str(e)}"
...
class GenerateVideoRequest(BaseModel):
    ...
    motion_strength: int = Field(127, ge=0, le=255, example=127)
...
async def api_generate_video(request: GenerateVideoRequest = Body(...)):
    ...
    try:
        ...
        # Call Modal function
        print("[API] Memanggil Modal function...")
        comfy_instance = ComfyUI()
        result = comfy_instance.generate_video.remote(
        ...
        print(f"[API] [ERROR] Timeout: {str(e)}")
        raise HTTPException(
            status_code=504,
            detail=f"Video generation timeout: {str(e)}"
        )
    except RuntimeError as e:
        print(f"[API] [ERROR] Runtime error: {str(e)}")
        raise HTTPException(
            status_code=500,
        ...
        error_msg = str(e)
        print(f"[API] [ERROR] Unexpected error: {error_msg}")
        import traceback
        traceback.print_exc()
        raise HTTPException(
            status_code=500,
