...
class ComfyUI:
    ...
    def _get_video_from_websocket(self, prompt_id: str, client_id: str) -> bytes:
                ...
                try:
                    ...
                    if isinstance(out, str):
                        ...
                        if message.get('type') == 'progress':
                            ...
                            # Log when nodes complete execution
                            data = message.get('data', {})
        ...
        # Give ComfyUI more time to finalize outputs
        ...
        raise ValueError(f"No video output found after {POLL_TIMEOUT}s. The workflow may have failed silently or saved to an unexpected location.")
...
if __name__ == "__main__":
    ...
    print("3. Deploy: modal deploy comfyui.py")
    ...
    print("=" * 80)
    print("\n1. ENVIRONMENT SETUP:")
    print("   - Ensure Modal account is configured")
    print("   - HuggingFace token may be required for model downloads")
    print("\n2. USAGE EXAMPLES:")
    print("\n   # Text-to-Video (Fast Mode)")
    print('   curl -X POST "https://your-app.modal.run/api/generate/t2v" \\')
    print('     -H "Content-Type: application/json" \\')
    print('     -d \'{"prompt": "A cat playing piano", "use_fast_mode": true}\'')
    print("\n   # Image-to-Video")
    print('   curl -X POST "https://your-app.modal.run/api/generate/i2v" \\')
    print('     -H "Content-Type: application/json" \\')
    print('     -d \'{"image_base64": "...", "prompt": "Make it dance"}\'')
    print("\n3. PERFORMANCE TIPS:")
    print("   - use_fast_mode=true: 4 steps with LoRA (faster)")
    print("   - use_fast_mode=false: 20-30 steps standard (higher quality)")
    print("   - Adjust num_frames for video length (more frames = longer generation)")
    print("\n4. MONITORING:")
    print("   - Check logs: modal app logs comfyui-wan2-2-complete-production")
    print("   - View dashboard: https://modal.com/apps")
    print("\n" + "=" * 80)
