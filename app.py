    ...
    def generate_t2v(
        ...
        try:
            ...
            if webhook_url and task_id:
                print(f"[WEBHOOK] Sending success result for {task_id} to: {webhook_url}")
                try:
                    payload = {"status": "success", "video_base64": video_base64, "task_id": task_id}
                    requests.post(webhook_url, json=payload, timeout=30)
                except Exception as e:
                    print(f"[WEBHOOK] Failed to send success result: {e}")
            
        except Exception as e:
            print(f"[ERROR] Generation failed for task {task_id}: {e}")
            if webhook_url and task_id:
                print(f"[WEBHOOK] Sending error result for {task_id} to: {webhook_url}")
                try:
                    payload = {"status": "error", "detail": str(e), "task_id": task_id}
                    requests.post(webhook_url, json=payload, timeout=30)
                except Exception as we:
                    print(f"[WEBHOOK] Failed to send error result: {we}")
            raise e
...
class T2VRequest(BaseModel):
    ...
    task_id: Optional[str] = Field(None, description="Unique ID for tracking the task")
...
async def api_generate_animate(request: AnimateRequest):
    try:
        ComfyUI().generate_animate.spawn(
...
async def api_apply_camera_lora(request: CameraLoraRequest):
    try:
        ComfyUI().apply_camera_lora.spawn(
            image_base64=request.image_base64,
            prompt=request.prompt,
