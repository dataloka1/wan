...
comfy_image = (
    ...
    .pip_install(
        ...
        "pydub>=0.25.1",  # Added for audio conversion
    )
...
"""
    ...
    def get_wan2_2_t2v_workflow(self, payload: dict) -> Dict:
        ...
        print(f"  - FPS: {fps}, Duration: {duration}s, Frames: {frames}")
        ...
        workflow = {
            ...
            "6": {"class_type": "WanEmptyLatentVideo", "inputs": {"width": width, "height": height, "length": frames, "batch_size": 1}},
        ...
        # Build LoRA chain if provided
        if loras:
            model_id, clip_id, lora_nodes = self._build_lora_chain("1", "3", loras)
            workflow.update(lora_nodes)
            
            # Update workflow to use LoRA outputs
            workflow["4"]["inputs"]["text_encoder"] = [clip_id, 0]
            workflow["5"]["inputs"]["text_encoder"] = [clip_id, 0]
            
            # Add sampler with LoRA model
            workflow["7"] = {"class_type": "WanSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "model": [model_id, 0], "positive": ["4", 0], "negative": ["5", 0], "latent_image": ["6", 0]}}
        else:
            # Add sampler without LoRA
            workflow["7"] = {"class_type": "WanSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "model": ["1", 0], "positive": ["4", 0], "negative": ["5", 0], "latent_image": ["6", 0]}}
    ...
    def get_wan2_2_i2v_workflow(self, payload: dict) -> Dict:
        ...
        if loras:
            ...
            # Update workflow to use LoRA outputs
            workflow["5"]["inputs"]["text_encoder"] = [clip_id, 0]
            workflow["6"]["inputs"]["text_encoder"] = [clip_id, 0]
            
            # Add sampler with LoRA model
            workflow["7"] = {"class_type": "WanI2VSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "frames": frames, "model": [model_id, 0], "positive": ["5", 0], "negative": ["6", 0], "image": ["3", 0]}}
        else:
            # Add sampler without LoRA
            workflow["7"] = {"class_type": "WanI2VSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "frames": frames, "model": ["1", 0], "positive": ["5", 0], "negative": ["6", 0], "image": ["3", 0]}}
    ...
    def get_wan2_2_s2v_workflow(self, payload: dict) -> Dict:
        ...
        if loras:
            ...
            # Add sampler with LoRA model
            workflow["7"] = {"class_type": "WanS2VSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "model": [model_id, 0], "positive": ["4", 0], "negative": ["5", 0], "latent_image": ["6", 0]}}
        else:
            # Add sampler without LoRA
            workflow["7"] = {"class_type": "WanS2VSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "model": ["1", 0], "positive": ["4", 0], "negative": ["5", 0], "latent_image": ["6", 0]}}
        
        return workflow
    
    def get_wan2_2_animate_workflow(self, payload: dict) -> Dict:
        print(f"\n[WORKFLOW] Building Animate workflow...")
        ...
        print(f"  - Input image: {'Yes' if image_base64 else 'No'}")
        print(f"  - LoRAs: {len(loras)}")
        print(f"  - Prompt: {prompt[:50]}...")
        ...
        if loras:
            ...
            if image_base64:
                print(f"[WORKFLOW] Mode: Image-to-Animation")
                image_path = self._save_base64_image(image_base64)
                workflow["6"] = {"class_type": "LoadImage", "inputs": {"image": image_path}}
                workflow["7"] = {"class_type": "WanAnimateSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "frames": frames, "motion_type": motion_type, "model": [model_id, 0], "positive": ["4", 0], "negative": ["5", 0], "image": ["6", 0]}}
            else:
                print(f"[WORKFLOW] Mode: Text-to-Animation")
                workflow["6"] = {"class_type": "WanEmptyLatentVideo", "inputs": {"width": width, "height": height, "length": frames, "batch_size": 1}}
                workflow["7"] = {"class_type": "WanAnimateSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "frames": frames, "motion_type": motion_type, "model": [model_id, 0], "positive": ["4", 0], "negative": ["5", 0], "latent_image": ["6", 0]}}
        else:
            if image_base64:
                print(f"[WORKFLOW] Mode: Image-to-Animation")
                image_path = self._save_base64_image(image_base64)
    ...
    def get_wan2_2_a2v_workflow(self, payload: dict) -> Dict:
        ...
        if not audio_base64:
            raise ValueError("'audio' parameter (base64) is required for A2V")
        ...
        # Base workflow
        workflow = {
            "1": {"class_type": "WanA2VModelLoader", "inputs": {"model_name": "wan2.2_i2v_low_noise_14B_fp16.safetensors"}},  # Using I2V model for A2V
            "2": {"class_type": "WanVAELoader", "inputs": {"vae_name": "wan_2.1_vae.safetensors"}},
            "3": {"class_type": "LoadAudio", "inputs": {"audio": audio_path}},
            "4": {"class_type": "WanT5TextEncoderLoader", "inputs": {"text_encoder_name": "umt5_xxl_fp8_e4m3fn_scaled.safetensors"}},
            "5": {"class_type": "WanT5TextEncode", "inputs": {"text": prompt, "text_encoder": ["4", 0]}},
            "6": {"class_type": "WanT5TextEncode", "inputs": {"text": negative_prompt, "text_encoder": ["4", 0]}},
            "8": {"class_type": "WanVAEDecode", "inputs": {"samples": ["7", 0], "vae": ["2", 0]}},
            "9": {"class_type": "VHS_VideoCombine", "inputs": {"frame_rate": fps, "loop_count": 0, "filename_prefix": "Wan2_2_A2V", "format": "video/h264-mp4", "crf": 18, "save_metadata": True, "pingpong": False, "save_output": True, "images": ["8", 0]}}
        }
        ...
        if loras:
            ...
            # Add sampler with LoRA model
            workflow["7"] = {"class_type": "WanA2VSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "frames": frames, "model": [model_id, 0], "positive": ["5", 0], "negative": ["6", 0], "audio": ["3", 0]}}
        else:
            # Add sampler without LoRA
            workflow["7"] = {"class_type": "WanA2VSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "frames": frames, "model": ["1", 0], "positive": ["5", 0], "negative": ["6", 0], "audio": ["3", 0]}}
        
        return workflow
    
    def get_wan2_2_controlnet_workflow(self, payload: dict) -> Dict:
        print(f"\n[WORKFLOW] Building ControlNet workflow...")
        ...
        if not image_base64:
            raise ValueError("'image' parameter (base64) is required for ControlNet")
        
        if not controlnets:
            raise ValueError("'controlnets' parameter is required for ControlNet")
        ...
        # Build ControlNet nodes
        positive_id, negative_id, controlnet_nodes = self._build_controlnet_nodes("3", controlnets, base_positive_id, base_negative_id)
        workflow.update(controlnet_nodes)
        
        # Add sampler with ControlNet
        workflow["100"] = {"class_type": "WanSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler_ancestral", "scheduler": "karras", "model": [base_model_id, 0], "positive": [positive_id, 0], "negative": [negative_id, 0], "latent_image": ["7", 0]}}
        
        workflow["101"] = {"class_type": "WanVAEDecode", "inputs": {"samples": ["100", 0], "vae": ["2", 0]}}
        workflow["102"] = {"class_type": "VHS_VideoCombine", "inputs": {"frame_rate": fps, "loop_count": 0, "filename_prefix": "Wan2_2_ControlNet", "format": "video/h264-mp4", "crf": 18, "save_metadata": True, "pingpong": False, "save_output": True, "images": ["101", 0]}}
        
        return workflow
    
    @method()
    def generate_t2v(self, payload: dict):
        ...
        workflow = self.get_wan2_2_t2v_workflow(payload)
    ...
    @method()
    def generate_i2v(self, payload: dict):
        ...
        client_id = str(uuid.uuid4())
        workflow = self.get_wan2_2_i2v_workflow(payload)
        prompt_id = self._queue_prompt(client_id, workflow)
        video_data = self._get_video_from_websocket(prompt_id, client_id)
    ...
    @method()
    def generate_s2v(self, payload: dict):
        """Generate video from story text"""
        print("\n" + "="*80)
        print("GENERATING STORY-TO-VIDEO")
        print("="*80)
        
        client_id = str(uuid.uuid4())
        workflow = self.get_wan2_2_s2v_workflow(payload)
        prompt_id = self._queue_prompt(client_id, workflow)
        video_data = self._get_video_from_websocket(prompt_id, client_id)
    ...
    @method()
    def generate_animate(self, payload: dict):
        """Generate animated video from text or image"""
        print("\n" + "="*80)
        print("GENERATING ANIMATED VIDEO")
        print("="*80)
        
        client_id = str(uuid.uuid4())
        workflow = self.get_wan2_2_animate_workflow(payload)
        prompt_id = self._queue_prompt(client_id, workflow)
        video_data = self._get_video_from_websocket(prompt_id, client_id)
    ...
    @method()
    def generate_a2v(self, payload: dict):
        """Generate video from audio and optional text prompt"""
        print("\n" + "="*80)
        print("GENERATING AUDIO-TO-VIDEO")
        print("="*80)
        
        client_id = str(uuid.uuid4())
        workflow = self.get_wan2_2_a2v_workflow(payload)
        prompt_id = self._queue_prompt(client_id, workflow)
        video_data = self._get_video_from_websocket(prompt_id, client_id)
    ...
    @method()
    def generate_controlnet(self, payload: dict):
        """Generate video with ControlNet guidance"""
        print("\n" + "="*80)
        print("GENERATING CONTROLNET VIDEO")
        print("="*80)
        
        client_id = str(uuid.uuid4())
        workflow = self.get_wan2_2_controlnet_workflow(payload)
        prompt_id = self._queue_prompt(client_id, workflow)
        video_data = self._get_video_from_websocket(prompt_id, client_id)
...
@web_app.post("/download_models")
async def api_download_models():
    """API endpoint to download models"""
    try:
...
async def api_generate_t2v(request: Request):
    """API endpoint for text-to-video generation"""
    try:
        payload = await request.json()
        comfyui = ComfyUI()
        video_data = comfyui.generate_t2v.remote(payload)
        return Response(content=video_data, media_type="video/mp4")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@web_app.post("/generate/i2v")
async def api_generate_i2v(request: Request):
    """API endpoint for image-to-video generation"""
    try:
        payload = await request.json()
        comfyui = ComfyUI()
        video_data = comfyui.generate_i2v.remote(payload)
        return Response(content=video_data, media_type="video/mp4")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@web_app.post("/generate/s2v")
async def api_generate_s2v(request: Request):
    """API endpoint for story-to-video generation"""
    try:
        payload = await request.json()
        comfyui = ComfyUI()
        video_data = comfyui.generate_s2v.remote(payload)
        return Response(content=video_data, media_type="video/mp4")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@web_app.post("/generate/animate")
async def api_generate_animate(request: Request):
    """API endpoint for animated video generation"""
    try:
        payload = await request.json()
        comfyui = ComfyUI()
        video_data = comfyui.generate_animate.remote(payload)
        return Response(content=video_data, media_type="video/mp4")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@web_app.post("/generate/a2v")
async def api_generate_a2v(request: Request):
    """API endpoint for audio-to-video generation"""
    try:
        payload = await request.json()
        comfyui = ComfyUI()
        video_data = comfyui.generate_a2v.remote(payload)
        return Response(content=video_data, media_type="video/mp4")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@web_app.post("/generate/controlnet")
async def api_generate_controlnet(request: Request):
    """API endpoint for ControlNet video generation"""
    try:
        payload = await request.json()
        comfyui = ComfyUI()
        video_data = comfyui.generate_controlnet.remote(payload)
        return Response(content=video_data, media_type="video/mp4")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def root():
    ...
    return {
        "message": "Wan2.2 Video Generation API",
        "endpoints": {
            "download_models": "/download_models",
            "generate_t2v": "/generate/t2v",
            "generate_i2v": "/generate/i2v",
            "generate_s2v": "/generate/s2v",
            "generate_animate": "/generate/animate",
            "generate_a2v": "/generate/a2v",
            "generate_controlnet": "/generate/controlnet"
        }
    }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python app.py [download_models|serve]")
        sys.exit(1)
    ...
    if command == "download_models":
        ...
        # This would be handled by Modal when deployed
        print("Server running on Modal")
    else:
