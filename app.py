...
def download_single_model(model_type: str, filename: str, url: str, target_dir: Path) -> dict:
    """Download single model with proper error handling"""
    ...
    try:
        print(f"  📥 [{model_type}] Downloading {filename}...")
        
        # Use wget with retry and better progress
        subprocess.run(
            [
                "wget",
                "-O", str(target_path),
...
def download_models():
    """Download semua model dengan 15 container paralel"""
    print("=" * 70)
    print("🚀 Starting Parallel Model Download (15 workers)")
    print("=" * 70)
    ...
    for model_type, models in MODEL_REGISTRY.items():
        ...
        for filename, url in models.items():
            download_tasks.append((model_type, filename, url, target_dir))
    
    print(f"\n📦 Total files to process: {len(download_tasks)}")
    print(f"⚡ Using 15 parallel workers\n")
    
    # Download with 15 parallel workers
    results = {
        "downloaded": [],
        "exists": [],
        "errors": []
    ...
    with ThreadPoolExecutor(max_workers=15) as executor:
        ...
        for future in as_completed(futures):
            ...
            if result["status"] == "downloaded":
                ...
                print(f"  ✓ [{completed}/{len(download_tasks)}] {result['filename']} (exists, {result['size']:.1f} MB)")
            else:
                results["errors"].append(result)
                print(f"  ❌ [{completed}/{len(download_tasks)}] {result['filename']}: {result.get('error', 'Unknown error')}")
    
    # Summary
    total_size = sum(r["size"] for r in results["downloaded"] + results["exists"])
    
    print("\n" + "=" * 70)
    print("📊 Download Summary:")
    print(f"   ✅ Downloaded: {len(results['downloaded'])} files")
    print(f"   ✓ Already exists: {len(results['exists'])} files")
    ...
    if results["errors"]:
        ...
        for err in results["errors"]:
            print(f"   - {err['filename']}: {err.get('error', 'Unknown')}")
...
def cli():
    ...
    print("   • 15 PARALLEL DOWNLOADS for faster setup")
    print("\n" + "=" * 70 + "\n")