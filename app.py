...
comfy_image = (
    ...
    .run_commands(
        ...
        f"python3.10 -m pip install --upgrade pip",
        f"cd {COMFYUI_PATH} && python3.10 -m pip install -r requirements.txt",
        # Wan Video Wrapper
        f"cd {COMFYUI_PATH / 'custom_nodes'} && git clone https://github.com/kijai/ComfyUI-WanVideoWrapper.git",
        f"cd {COMFYUI_PATH / 'custom_nodes/ComfyUI-WanVideoWrapper'} && python3.10 -m pip install -r requirements.txt",
        # Video Helper Suite
        f"cd {COMFYUI_PATH / 'custom_nodes'} && git clone https://github.com/Kosinkadink/ComfyUI-VideoHelperSuite.git",
        f"cd {COMFYUI_PATH / 'custom_nodes/ComfyUI-VideoHelperSuite'} && python3.10 -m pip install -r requirements.txt",
        # ControlNet Aux
        f"cd {COMFYUI_PATH / 'custom_nodes'} && git clone https://github.com/Fannovel16/comfyui_controlnet_aux.git",
        f"cd {COMFYUI_PATH / 'custom_nodes/comfyui_controlnet_aux'} && python3.10 -m pip install -r requirements.txt",
    )
    .pip_install("websocket-client", "safetensors")
)
...
"""
        ...
        cmd = f"python3.10 main.py --listen 0.0.0.0 --port 8188 --disable-auto-launch"
    ...
    def get_wan2_2_premium_workflow(self, payload: dict) -> Dict:
        ...
        steps = payload.get("steps", 80)  # Higher steps untuk 30s video
        cfg = payload.get("cfg", 7.5)
        width = payload.get("width", 1280)  # 1280x720 untuk high quality
        height = payload.get("height", 720)
        
        # 30 detik dengan 24fps = 720 frames (cinematic)
        fps = payload.get("fps", 24)
        duration = payload.get("duration", 30)  # seconds
        frames = fps * duration  # 24 * 30 = 720 frames
        
        # Lora stacking untuk hasil terbaik
        loras = payload.get("loras", [
            {"name": "Cinematic_V2.safetensors", "strength": 0.8},
            {"name": "4k_Hyperrealistic_Style.safetensors", "strength": 0.7},
            {"name": "add_detail.safetensors", "strength": 0.6},
            {"name": "film-grain.safetensors", "strength": 0.4},
        ])
        
        # Camera motion lora (optional)
        camera_motion = payload.get("camera_motion", "None")  # ZoomIn, PanLeft, etc.
        ...
        workflow = {
            ...
            "9": {
                ...
                "inputs": {
                    ...
                    "crf": 18,  # Low CRF untuk quality tinggi
                    "save_metadata": True,
                    "pingpong": False,
                    "save_output": True,
                    "images": ["8", 0]
...
def fastapi_app():
    ...
    async def root():
        return {
            ...
            "features": [
                ...
                "✅ 1280x720 HD output",
                "✅ 10 concurrent containers with auto-queue"
            ],
            "endpoints": {
    ...
    async def generate(request: Request):
        ...
        Example payload:
        {
            "prompt": "A cinematic shot of a futuristic city at sunset, neon lights, cyberpunk aesthetic, 8k, ultra detailed",
            ...
            "loras": [
                {"name": "Cinematic_V2.safetensors", "strength": 0.8},
                {"name": "Cyberpunk_Style.safetensors", "strength": 0.7}
            ],
        ...
        try:
            ...
            # Set defaults untuk 30s premium video
...
def cli():
    ...
    print("   • 10 concurrent containers with auto-queue")
