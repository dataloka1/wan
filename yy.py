...
def download_models_task(required_only: bool = False):
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info(f"✅ DOWNLOAD COMPLETE - {successful}/{total} models")
        logger.info("=" * 80)
...
@modal.concurrent(max_inputs=LimitsConfig.MAX_CONCURRENT_REQUESTS)
...
def main(command: str = "setup"):
    ...
    print("=" * 80)
    print("🎬 Wan2.2 Video Generation Service")
    print("=" * 80)
    ...
    if command == "setup":
        ...
        print("\n📦 Downloading required models only...")
        ...
        if result["status"] == "completed":
            ...
            print(f"\n❌ Test generation failed: {result.get('error')}")
